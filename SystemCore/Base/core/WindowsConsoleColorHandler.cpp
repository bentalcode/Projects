...
HANDLE WindowsConsoleColorHandler::getFileDescriptorHandle(StandardFileDescriptor fileDescriptor)
{
    ...
    {
        assert(false);
        std::wstring errorMessage = L"The standard file descriptor is not supported.";
        throw NotImplementedException(errorMessage);
    }
}
...
unsigned short WindowsConsoleColorHandler::GetForegroundColorAttributes(ColorType colorType) const
{
    ...
    {
        long errorCode = ErrorCodes::RESOURCE_NOT_FOUND;

        std::wstringstream errorMessageStream;
        errorMessageStream
            << L"The color type for the specific foreground color is not defined"
