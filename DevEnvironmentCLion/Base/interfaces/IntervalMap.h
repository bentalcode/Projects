...
namespace base {
    ...
    private:
        typedef std::map<K, V> IntervalCollection;
         ...
         */
        typename std::map<K, V>::const_iterator findNextSmallerOrEqualKey(const K& key) const;
    ...
    const V& IntervalMap<K, V>::operator[](const K& key) const {
        ...
        //
        typename IntervalCollection::const_iterator iterator = findNextSmallerOrEqualKey(key);
    ...
    bool IntervalMap<K, V>::findEndKeyValue(const K& key, V& result) const {
        typename IntervalCollection::const_iterator i = findNextSmallerOrEqualKey(key);
     ...
     */
    template<typename K, typename V>
    typename IntervalMap<K, V>::IntervalCollection::const_iterator IntervalMap<K, V>::findNextSmallerOrEqualKey(const K& key) const {
        if (m_map.empty()) {
            return m_map.end();
        }
        ...
        //
        typename IntervalCollection::const_iterator iterator = m_map.lower_bound(key);
    ...
    inline void IntervalMap<K, V>::removeKeyRange(const K& keyBegin, const K& keyEnd) {
        typename IntervalCollection::iterator startIterator = m_map.lower_bound(keyBegin);
        typename IntervalCollection::iterator endIterator = m_map.upper_bound(keyEnd);

        typename IntervalCollection::iterator iterator = startIterator;
